...
PATH = 'model/transGRU_'+country+'.pth'
...
# D = torch.tensor(checks[:, 1:training_days, 0:1])
# R = torch.tensor(checks[:, 1:training_days, 1:2])
# A = torch.tensor(checks[:, 1:training_days, 2:3])
...
class GRUNet(nn.Module):
    ...
    def forward(self, x, D_ins, R_ins, A_ins, h):
        ...
        # I = ((self.alpha + self.gamma)*D + D_ins)/self.d_iq
        # zeroTensor = torch.tensor(np.zeros([1, 1, 1])).float().to(device)
        # I_ins =torch.cat((zeroTensor, I), 1)[:, :-1, :]
        # I_ins = I - I_ins
    ...
    def getParameter(self):
        print(self.alpha, self.gamma, self.d_iq)
...
def train(train_loader, learn_rate, hidden_dim=hidden_dim, num_epochs=epoch):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch % 100 == 0:
            print(
                "Epoch {}/{} Done, Total Loss: {}, min Loss: {}".format(epoch, num_epochs, avg_loss / len(train_loader),min_loss))
            # print(e_re)
            # model.getParameter()
